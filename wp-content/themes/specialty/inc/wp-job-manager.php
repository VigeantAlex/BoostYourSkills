<?php
	...
	if ( ! function_exists( 'specialty_wpjb_get_the_job_types' ) ) {
		function specialty_wpjb_get_the_job_types( $post = null ) {
			$post = get_post( $post );
			...
			foreach ( $types as $key => $type ) {
				$types[ $key ] = apply_filters( 'the_job_type', $type, $post );
			}
	...
	if ( ! function_exists( 'specialty_wpjb_the_job_types' ) ) {
		function specialty_wpjb_the_job_types( $before = '', $after = '', $post = null ) {
			$types = specialty_wpjb_get_the_job_types( $post );

			foreach ( $types as $type ) {
				echo $before . $type->name . $after;
			}
		}
	}
